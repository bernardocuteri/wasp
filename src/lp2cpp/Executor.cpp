...
void Executor::executeProgramOnFacts(const std::vector<int> & facts) {
    ...
    std::cout<<"Execute program on facts: decision level "<<decisionLevel<<std::endl;
    clearPropagations();
    for(unsigned i=1;i<facts.size();i++) {
        std::cout<<"facts: "<<facts[i]<<std::endl;
    ...
    if(decisionLevel==-1) {
        {
            ...
            for( unsigned i=0; i< tuples->size() + tuplesU->size();i++){
                ...
                int X_1 = (*tuple0)[1];
                if( (*tuple0)[0] == 4){
                    std::cout<<"Decision Level If0"<<std::endl;
                    ...
                    std::cout<<"trueSize map "<<joinTuples_0_1SharedVariables.first->size()<<std::endl;
                    std::cout<<"undefSize map "<<joinTuples_0_1SharedVariables.second->size()<<std::endl;
                    if(tupleU==NULL){
                        std::cout<<"undefSize "<<actualSize1-joinTuples_0_1SharedVariables.first->size()<<std::endl;
                        if(!(actualSize1>=0)){
                    ...
                    std::cout<<"Undef not in aggregate"<<std::endl;
                        if(!(actualSize1>=0)){
                            const auto & it = tupleToVar.find(*tupleU);
                            if(it != tupleToVar.end()) {
                                std::cout<<"External prop";tupleU->print();std::cout<<std::endl;
        ...
        }//close local scope
        {
            ...
            for( unsigned i=0; i< tuples->size() + tuplesU->size();i++){
                ...
                int X_1 = (*tuple0)[1];
                if( (*tuple0)[0] == 4){
                    std::cout<<"Decision Level If1"<<std::endl;
                    ...
                    std::cout<<"trueSize map "<<joinTuples_1_1SharedVariables.first->size()<<std::endl;
                    std::cout<<"undefSize map "<<joinTuples_1_1SharedVariables.second->size()<<std::endl;
                    if(tupleU==NULL){
                    ...
                    std::cout<<"Undef not in aggregate"<<std::endl;
                        if(joinTuples_1_1SharedVariables.first->size()>=0){
                            const auto & it = tupleToVar.find(*tupleU);
                            if(it != tupleToVar.end()) {
                                std::cout<<"External prop";tupleU->print();std::cout<<std::endl;
                                int sign = tupleU->isNegated() ? -1 : 1;
        ...
        if(facts[i] > 0){
            ...
            if(starter.getPredicateName()== &_p_4 || starter.getPredicateName()== &_p_1 || starter.getPredicateName()== &_p_0){
                for(const auto sharedVarTuple : sharedVariables_1_ToAggregate_1){
                    ...
                    std::cout<<"trueSize map "<<sharedVarTuple.second->first->size()<<std::endl;
                    std::cout<<"undefSize map "<<sharedVarTuple.second->second->size()<<std::endl;
                        ...
                        if(tuple1){
                            if( (*tuple1)[0] == 4){
                                ...
                                if(tupleU == NULL) {
                                    ...
                                    const auto & it = tupleToVar.find(*tupleU);
                                    if(it != tupleToVar.end()) {
                                        std::cout<<"External propagation";tupleU->print();std::cout<<std::endl;
                                        int sign = tupleU->isNegated() ? -1 : 1;
                            ...
                            if(tuple1){
                                if( (*tuple1)[0] == 4){
                                    if(tupleU ==  NULL) {
                                        ...
                                        if(it_reason1!=tupleToVar.end())
                                            reason.push_back(it_reason1->second * (tuple1->isNegated() ? -1:1));
                                        std::cout<<"propagating Aggregate starting from aggregate"<<std::endl;
                                        const std::vector<const Tuple*>* undefinedTuples = &u_p_4_X_4_X_1_p_1_X_0_X_1_p_0_X_3_1_3_.getValues({X_1,X_1});
                                        for(int iUndef=0;iUndef<undefinedTuples->size();iUndef++){
                                            if(joinTuples_1_1SharedVariables.first->getValues({undefinedTuples->at(iUndef)->at(4)}).size() == 0){
                                                const Tuple* aggrTupleU=NULL;
            ...
            if(facts[i] > 0){
                {
                    ...
                    int X_1 = (*tuple0)[1];
                    if( (*tuple0)[0] == 4){
                        std::cout<<"For on Facts0"<<std::endl;
                        std::pair<AuxMap*,AuxMap*> joinTuples_0_1;
                        joinTuples_0_1.first = &p_p_4_X_4_X_1_p_1_X_0_X_1_p_0_X_3_;
                        joinTuples_0_1.second = &u_p_4_X_4_X_1_p_1_X_0_X_1_p_0_X_3_;
                        std::pair<AuxMap*,AuxMap*> joinTuples_0_1SharedVariables = *sharedVariables_0_ToAggregate_1[std::vector<int>({X_1,X_1})];
            ...
            if(facts[i] > 0){
                {
                    ...
                    int X_1 = (*tuple0)[1];
                    if( (*tuple0)[0] == 4){
                        std::cout<<"For on Facts1"<<std::endl;
                        std::pair<AuxMap*,AuxMap*> joinTuples_1_1;
                        joinTuples_1_1.first = &p_p_4_X_4_X_1_p_1_X_0_X_1_p_0_X_3_;
                        joinTuples_1_1.second = &u_p_4_X_4_X_1_p_1_X_0_X_1_p_0_X_3_;
                        std::pair<AuxMap*,AuxMap*> joinTuples_1_1SharedVariables = *sharedVariables_1_ToAggregate_1[std::vector<int>({X_1,X_1})];

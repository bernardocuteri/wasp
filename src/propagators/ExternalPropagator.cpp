...
#include "../interpreters/MyCppEagerInterpreter.h"
    ...
    string scriptDirectory ) : Propagator()
{
    ...
    else if(interpr == CPP_LAZY_INTERPRETER) 
        interpreter = new MyCppLazyInterpreter( fn, scriptDirectory );
    else if(interpr == CPP_EAGER_INTERPRETER) 
        interpreter = new MyCppEagerInterpreter( fn, scriptDirectory );
    else
...
void 
ExternalPropagator::onFact(Var var) {
    if( check_onFact ) {
        interpreter->callVoidMethod( method_plugins_onFact, var);
    }
}
